...
int main(void)
{
  ...
  CMW_Sensor_Name_t sensor_name;
  CMW_CAMERA_GetSensorName(&sensor_name);

  printf("Camera sensor enum: %d\r\n", sensor_name);

  switch (sensor_name)
  {
    case CMW_VD55G1_Sensor:
      printf("Camera sensor: VD55G1\r\n");
      break;

    case CMW_VD65G4_Sensor:
      printf("Camera sensor: VD65G4\r\n");
      break;

    case CMW_IMX335_Sensor:
      printf("Camera sensor: IMX335\r\n");
      break;

    case CMW_OV5640_Sensor:
      printf("Camera sensor: OV5640\r\n");
      break;

    default:
      printf("Camera sensor: UNKNOWN / OTHER\r\n");
      break;
  }
  Display_init();
...
static bool Claritone_IsPersonClass(int class_id)
{
  ...
  if (classes_table[class_id] == NULL)
  {
    return false;
  }

  return (strcmp(classes_table[class_id], "person") == 0);
}

static claritone_target_t Claritone_SelectBestPerson(
    const od_pp_out_t *p_postprocess)
{
...
static void Claritone_HandleFrame(const od_pp_out_t *p_postprocess)
{
  static claritone_zone_t last_zone = -1;

  claritone_target_t best_person =
      Claritone_SelectBestPerson(p_postprocess);

  if (best_person.valid)
  {
    if (best_person.zone != last_zone)
    {
      printf("PERSON DETECTED -> %s\r\n",
             Claritone_ZoneToString(best_person.zone));

      last_zone = best_person.zone;
    }
  }
}
